...
export function TopicForm() {
    ...
    useEffect(() => {
        if (!isEdit) return;
        api.topics.get(Number(id))
            .then((t: Topic) => setForm({ title: t.title, description: t.description, age_min: t.age_min, age_max: t.age_max, status: t.status }))
    ...
    async function handleSubmit(e: FormEvent) {
        ...
        try {
            if (isEdit) {
                await api.topics.update(Number(id), form);
            } else {
    ...
    return (
        <div className="max-w-xl mx-auto space-y-4">
            <h1 className="text-2xl font-bold">{isEdit ? 'Edit Topic' : 'New Topic'}</h1>
            ...
            <div className="card bg-base-100 shadow">
                <form onSubmit={handleSubmit} className="card-body gap-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <label className="form-control">
                            <div className="label"><span className="label-text">Min Age</span></div>
                            <input type="number" className="input input-bordered" min={1} max={18} value={form.age_min} onChange={e => setForm(f => ({ ...f, age_min: Number(e.target.value) }))} />
                        </label>
                        <label className="form-control">
                            <div className="label"><span className="label-text">Max Age</span></div>
                            <input type="number" className="input input-bordered" min={1} max={18} value={form.age_max} onChange={e => setForm(f => ({ ...f, age_max: Number(e.target.value) }))} />
                        </label>
                    </div>
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                        <button type="button" onClick={() => navigate('/cms/topics')} className="btn btn-ghost">Cancel</button>
